...
class ImageConverter():
    ...
    def OpenFile(self, temp_file_path):
        ...
        if (str(temp_file_path) == ""):
            ...
            filename, file_extension = os.path.splitext(temp_file_path) #get type from file name
            #print(filename)
            #print(file_extension)
            if (file_extension.lower() == '.svg'):
                self.file_type = 2 #set image type to vector
                #print('set to vector')
            else:
                self.file_type = 1 #set image type to bitmap
                #print('set to bitmap')
                
            temp_success = 0 
            #open bitmap file
            if (self.file_type == 1): #attempt opening bitmap file
                try: #try opening file
                    #self.input_image = QPixmap(str(temp_file_path))
                    self.conversion_image = QImage(str(self.file_path)) #create conversion image
                    self.input_image = QPixmap(self.conversion_image) #create input image
                    self.output_image = QPixmap(self.conversion_image) #create output image
                ...
                if (temp_success == 1):
                    ...
                    self.image_array_width = self.conversion_image.width()
                    self.image_array_height = self.conversion_image.height()
                    #print("Image width: " + str(self.image_array_width) + ", Image height: " + str(self.image_array_height))
                    self.image_array = zeros( (self.image_array_height, self.image_array_width) )
                    #print(self.image_array)
                    return 1
                    
            if (self.file_type == 2): #attempt opening vector file
                try: #try opening file
                ...
                if (temp_success == 1):
                    #print("file opened")
                    self.SVGGetData() #get data from vector file
    ...
    def Threshold(self, temp_threshold):
        ...
        for w in range(0,self.image_array_width):
            for h in range(0,self.image_array_height):
                self.pixel_color = self.conversion_image.pixelColor(w,h) #get pixel color
                ...
                self.falpha = float(self.alpha / 255)
                self.finfalpha = 1.0 - self.falpha
                
                self.background = 255 * self.finfalpha
                self.gray = (self.qred+self.qgreen+self.qblue)/3
                self.gray *= self.falpha
                self.gray = self.gray + self.background
                if (self.gray < temp_threshold):
                    self.image_array[h][w] = 1
                else:
    ...
    def ArrayToImage(self):
        ...
        for w in range(0,self.image_array_width):
            for h in range(0,self.image_array_height):
                if(self.image_array[h,w] == 0):
                    self.temp_color.setRgb(255,255,255)
                elif(self.image_array[h,w] == 1):
                    self.temp_color.setRgb(0,0,0)
                    ...
                    while(True):
                        ...
                        if (temp_decode[0].lstrip() == 'id='): #get layer data
                            ...
                            if (temp_layer_name == self.svg_layer_names[temp_layer]):
                                temp_line_found = 1
                                temp_toggle_array = zeros( (self.image_array_height, self.image_array_width) ) #clear image array
                                self.image_array = zeros( (self.image_array_height, self.image_array_width) ) #clear image array
                                #print(temp_layer_name)
